...
class CsvAPI(FileAPI):
    def read_csv(self, quantity: str) -> DataFrame:
        with TextIOWrapper(self.read(quantity=quantity, format="csv")) as csv_file:
            return read_csv(csv_file)

    def write_csv(self, quantity: str, value: DataFrame):
        with TextIOWrapper(self.write(quantity=quantity, run_id=1, extension="csv")) as csv_file:
