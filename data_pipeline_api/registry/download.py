...
def _get_data_product_version_and_components(
    ...
    data_product_versions = sort_by_semver(data_product_versions)
    if parsed_config.component_name:
        ...
        for dpv in data_product_versions:
            data_product_version_components = [
                cn
                for cn in [
                    get_on_end_point(c, token)[DataRegistryField.name] for c in dpv[DataRegistryField.components]
                ]
        ...
        data_product_version_components = [
            ...
            for c in data_product_version[DataRegistryField.components]
        ]
    logger.info(
...
def _download_data(output_info: OutputInfo) -> None:
    ...
    if output_info.accessibility == "public":
        ...
        fs, source_path = get_remote_filesystem_and_path(
            output_info.source_protocol, output_info.source_uri, output_info.source_path
        )
        if output_info.source_protocol == "file":
            Path(source_path).parent.mkdir(parents=True, exist_ok=True)
        fs.get(source_path, output_info.output_path)
    else:
